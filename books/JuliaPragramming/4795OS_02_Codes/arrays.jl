...
arr = [100, 25, 37]
arra = Any[100, 25, "ABC"] #> 원소 타입은 Any이다
arr[1] #> 100
arr[end] #> 37
# arr[6] #> BoundsError
...
b = [1:7]
c = [100,200,300]
append!(b,c) # b는 [1, 2, 3, 4, 5, 6, 7, 100, 200, 300]이다
pop!(b) #> 300, b는 [1, 2, 3, 4, 5, 6, 7, 100, 200]이다
push!(b, 42) # b는 [1, 2, 3, 4, 5, 6, 7, 100, 200, 42]이다
shift!(b) #> 1, b는 [2, 3, 4, 5, 6, 7, 100, 200, 42]이다
unshift!(b, 42) # b는 [42, 2, 3, 4, 5, 6, 7, 100, 200, 42]이다
splice!(b,8) #> 100, b는 [42, 2, 3, 4, 5, 6, 7, 200, 42]이다
in(42, b) #> true
in(43, b) #> false
println()
println("sorting:")
sort(b) #> [2,3,4,5,6,7,42,42,200], b는 변경되지 않는다
println(b) #> [42,2,3,4,5,6,7,200,42]
sort!(b) #> b는 [2,3,4,5,6,7,42,42,200]로 변경되었다
